...
from services.engineer_service import get_associated_user
...
class ReportSection:
    ...
    def build(self) -> List[ft.Control]:
        ...
        sort_popup = ft.PopupMenuButton(
            ...
            items=[
                ...
                ft.PopupMenuItem(
                    content="Severity", on_click=lambda _: self.change_sort("Severity")
                ),
                ft.PopupMenuItem(
                    content="Probability",
                    on_click=lambda _: self.change_sort("Probability"),
                ),
            ],
        )
        ...
        group_popup = ft.PopupMenuButton(
            ...
            items=[
                ft.PopupMenuItem(
                    content="User", on_click=lambda _: self.change_group("User")
                ),
                ft.PopupMenuItem(
                    content="Date", on_click=lambda _: self.change_group("Date")
                ),
                ft.PopupMenuItem(
                    content="Severity", on_click=lambda _: self.change_group("Severity")
                ),
            ],
        )
    ...
    def update_report(self):
                ...
                controls=[
                    ...
                    ft.Container(
                        ...
                        content=ft.Column(
                            controls=[
                                ft.Icon(
                                    ft.Icons.SEARCH_OFF, size=64, color=ft.Colors.GREY
                                ),
                                ft.Text(
                                    "No cracks match the filter.",
                                    size=16,
        ...
        if self.current_sort == "Date Descending":
            ...
            cracks = sorted(
                self.filtered_cracks, key=lambda c: c.get("detected_at", "")
            )
        elif self.current_sort == "Severity":
            order = {"High": 0, "Mild": 1, "Low": 2}
            cracks = sorted(
                self.filtered_cracks, key=lambda c: order.get(c.get("severity", ""), 99)
            )
        ...
        for crack in cracks:
            if self.current_group == "User":
                ...
                try:
                    key = datetime.datetime.strptime(raw_date, "%Y-%m-%d").strftime(
                        "%B %d, %Y"
                    )
                except ValueError:
                    key = raw_date
        ...
        for group_name, group_cracks in ordered_groups.items():
            ...
            for crack in group_cracks:
                ...
                color = {
                    "High": ft.Colors.RED,
                    "Mild": ft.Colors.YELLOW,
                    "Low": ft.Colors.GREEN,
                }.get(severity, ft.Colors.GREY)

                thumb = build_thumb(crack, with_playbtn=False)
                thumb.on_click = lambda _, f=crack: show_full(
                    self.page, f, refresh_function=self.refresh
                )
                ...
                self.report_list.controls.append(
                    ft.ListTile(
                        ...
                        subtitle=ft.Column(
                            controls=[
                                ft.Text(
                                    f"{severity} — {probability * 100:.1f}%", size=13
                                ),
                                ft.Text(
                                    f"{username}  ·  {date} {time}",
                                    size=12,
                                    color=ft.Colors.GREY,
                                ),
                            ],
                        ...
                        on_click=lambda _, f=crack: show_full(
                            self.page, f, refresh_function=self.refresh
                        ),
                    )
                )
